...
- Q: Should `skills` be deduplicated across the whole roadmap, or scoped independently per node? → A: Each node's `skills` is determined by feature 003. Deduplication is a presentation concern owned by Feature 004.
...
When a student submits their full onboarding profile (including career goal, completed courses, and other optional fields), the system asynchronously generates a personalised learning roadmap. The roadmap contains a career-targeted subset of courses — all required prerequisite courses plus only the electives the AI determines are most relevant to the student's stated role and company type — ordered to respect all academic prerequisites. Each course node is enriched with a list of skills acquired from the course and a plain-language explanation of why the course matters for their specific career goal. Upon completion, the student is presented with a roadmap preview; the roadmap is committed as their active roadmap only after they explicitly accept it.
...
**Independent Test**: Can be fully tested by triggering generation with a complete StudentProfile (all fields filled), then verifying the stored Roadmap document contains: only career-relevant courses in valid prerequisite order, no completed courses as actionable nodes, and every node having a non-empty skills array, a reason, and an empty resources array.
...
5. **Given** every selected node is ordered, **When** the AI enriches each node, **Then** each node has an empty skills array (to be enriched by Feature 003), a reason, and an empty resources array.
...
A student submits their onboarding with only the required major field filled in and no career goal provided. The system generates a generic roadmap covering the core required courses of that major in valid prerequisite order. The roadmap is explicitly flagged as low-personalisation so the downstream Skill Tree can surface a notice prompting the student to enrich their profile.
...
**Independent Test**: Can be fully tested by triggering generation with a StudentProfile containing only a major (all optional fields empty), then verifying the stored Roadmap document contains all required courses for that major, in valid prerequisite order, with a low-personalisation flag set, and every node having a non-empty skills array.
...
3. **Given** courses are selected in generic mode, **When** each node is enriched, **Then** skills are generic best-practice recommendations.
...
- **FR-013**: Each roadmap node MUST include a `skills` field: a list of skills and concepts the student directly acquires by completing the course. This field MUST contain only skills fully covered by the course content — cross-node deduplication is not required and is left to the rendering layer (Feature 004).
...
- **FR-019**: In the low-personalisation pathway, `skills` for each node MUST contain generic best-practice recommendations rather than role-targeted skills.
...
- **FR-033c**: During migration, `GET /api/roadmaps/primary` MAY be kept as a compatibility alias for `GET /api/primary-roadmap` and MUST be documented as deprecated.
...
- **RoadmapNode**: A single course entry in the ordered roadmap. Fields:
  - `courseCode`, `courseName`, `credits`, `suggestedSemester`
  - `skills`: Array of skills (enriched by Feature 003)
  - `reason`: Explanation of relevance (generated by Feature 009)
...
- **SC-007**: Every node in every generated roadmap contains a non-empty `resources` and 'skills' field (as an empty array) — Feature 003 can populate `resources` at any time without requiring re-generation or schema changes but 'skills' is fixed after generation.
...
- Resource curation and population of the `resources` and 'skills' field on roadmap nodes — owned by Feature 003
