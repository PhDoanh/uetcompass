...
## Decision 2: Onboarding Section Behavior in Account Settings

- Decision: Account Settings onboarding section is mode-based:
  - `mode = cta` when onboarding not completed: show button to open Onboarding Panel.
  - `mode = editable` when onboarding completed: render onboarding-derived fields for editing.
- Rationale: Matches user requirement exactly while preserving current completed-onboarding edit behavior.
- Alternatives considered:
  - Hide section before onboarding completion (rejected: user explicitly requested visible button).
  - Always show editable fields (rejected: conflicts with onboarding flow ownership in Feature 001).

## Decision 3: Soft Delete for Account Deletion
...
## Decision 4: Password Change Security Baseline
...
## Decision 5: Unified Identity Rendering and Privacy Enforcement
...
## Decision 6: Audit Events for Sensitive Changes
