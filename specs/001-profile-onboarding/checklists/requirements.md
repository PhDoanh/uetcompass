...
- **FR-006/006a/009a/009b/009c** are now explicitly data-contract aligned with feature 002: major display from `programs.nameEN`, curriculum link from `course_units.source.url` (matched by `programId`), and completed-course options from `course_units` filtered by (`programId`, `type = "elective"`).
...
- [x] **US4 — Role dropdown + graduation date-picker selection + validation**
...
- [x] Target role dropdown source aligned to `programs.careerTracks` for selected `programId`.
- [x] "Required Courses" link target aligned to `course_units.source.url` from any row with selected `programId`.
